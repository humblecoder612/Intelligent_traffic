...
def full_darknet_body(x, net_type, is_training):
    ...
    if net_type in ['cnn', 'mobilenetv1']:
        ...
        x = conv_block(x, [3, 3], [2, 2], 16, net_type, is_training=is_training)
        x = conv_block(x, [3, 3], [1, 1], 24, net_type, is_training=is_training) 
        x = residual(x, net_type, is_training, 24, 1)
        x = conv_block(x, [3, 3], [2, 2], 32, net_type, is_training=is_training) 
        ...
        for i in range(3): 26×26×64  
            x = residual(x, net_type, is_training, 64, 6)
        x = conv_block(x, [3, 3], [1, 1], 96, net_type, is_training=is_training)#
        for i in range(2):
        ...
        x = conv_block(x, [3, 3], [1, 1], 320, net_type, is_training=is_training)

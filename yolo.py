...
class YOLO():
    ...
    def forward(self, img,path):
        ...
        for b in range(self.batch_size):
            picked_boxes = pick_box(boxes[b], 0.5, 0.6, self.hw, self.classes)
            ...
            with open(path+".txt", "w") as text_file:
                    print("{}".format(len(true_boxes)), file=text_file)
            #print(true_boxes)
            vis_img.append(per_img)
        return vis_img[0],len(true_boxes)
    ...
    def detect_video(self, video_path):
        ...
        while True:
            ret, frame = cap.read()
            if ret:
                out,boxy = self.forward(frame,video_path)
                time2 = time.time()
                d_time = time2 - time1
                time1 = time2
                total_time += d_time
                ...
                out = cv2.putText(out, fps+' ' +str(boxy), tuple(np.int32([20, 30])),
